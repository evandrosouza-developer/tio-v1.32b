...
static bool copy_two_lines_of_X(uint8_t*x, long*iter, long*fileSize, FILE *fp)
{
  ...
  //cursor positioning to lower serigraphy
  printf("\x1B[%d;%dH", 3*(*x)+2+page, 0);
  printf("%s", mark);  //"X="
  ch_file_in = fgetc(fp);
  (*iter)++;
  if(*iter > *fileSize)
  ...
  mark[3] = 0; mark[2] = 0; mark[1] = 0; mark[0] = 0;
  y = 0;
  //cursor positioning to upper serigraphy
  while((ch_file_in != '\n') && (ch_file_in != '\r') && (y < 12))
  {
    //printf("\x1B[%d;%dH", 3*(*x)+2+page, 6*y+5);
    //Search for "; $"
...
bool load_key_matrix(void)
{
  ...
  if ((fp=fopen (fname,"r")) != NULL)
  {
    ...
    /* read the 8 MSX lines */
    for(x = 0; x < 7; x++)
    {
      /*Two lines of X*/
      if(!copy_two_lines_of_X(&x, &iter, &fileSize, fp))
      {
        fclose(fp);
        return false;
      }
    ...
    /*X=7, that does NOT have the third line*/
    x = 7;
    if(!copy_two_lines_of_X(&x, &iter, &fileSize, fp))
    {
      fclose(fp);
...
#define LIGHT_BLUE  36
  ...
  {
    //This bit (key) is pressed. Set to marked. Compute screen position
    printf("\x1B[%d;%dH", 3 * which_x_bit + 2, 6 * y + 5);//upper serigraphy
    printf("\x1B[%d;%dm%s\x1B[m", NEGATIVE, LIGHT_BLUE, &key_matrix[which_x_bit][y][0][0]);
    printf("\x1B[%d;%dH", 3 * which_x_bit + 3, 6 * y + 5);//lower serigraphy
    printf("\x1B[%d;%dm%s\x1B[m", NEGATIVE, LIGHT_BLUE, &key_matrix[which_x_bit][y][1][0]);
  }
  else
  {
    //This bit (key) is released. Set to unmarked
    printf("\x1B[%d;%dH", 3 * which_x_bit + 2, 6 * y + 5);//upper serigraphy
...
void check_input_char(char input_char, char*mount_string)
{
  char *p;
    ...
    if( (p != NULL) && (*(p+2) == ' ') )
    {
      ...
      if(((ch > ('0' - 1) && ch < ('9' + 1)) || (ch > ('A' - 1) && ch < ('F' + 1))))
      {
        ...
        if(p != NULL)
        {
          ...
          printf("\x1B[%d;%dH", 23, 0); //Moves the cursor to last line
        } //if(p != NULL)
      } //if((ch > ('0' - 1) && ch < ('9' + 1)) || (ch > ('A' - 1) && ch < ('F' + 1)))
    } //if(p != NULL)
    mount_string[0] = 0;
...
void file_send(void)
{
  ...
  if ((fp=fopen (fname,"r")) != NULL)
  {
    ...
    {
      error_printf("Failed to add STDIN_FILENO file descriptor to epoll\n");
      close(epoll_fd);
      exit(EXIT_FAILURE);
    }

    ev.data.fd = tty_fd;
    if(epoll_ctl(epoll_fd, EPOLL_CTL_ADD, tty_fd, &ev))
    {
      error_printf("Failed to add tty_fd file descriptor to epoll\n");
      close(epoll_fd);
      exit(EXIT_FAILURE);
    }
    ...
    while(iter < fileSize)
    {
      ...
      nds = epoll_wait(epoll_fd, events, MAX_EVENTS, 0);
      ...
      if(x_on_state)
      {
        ...
        status = write(tty_fd, &ch_file_in, 1); //No need to use epoll here, as it will block if not ready
        if(status >= 0)
        {
          tx_total ++;
    ...
    fclose(fp);
    warning_printf("File send concluded!\r\n");
  } //if ((fp=fopen (fname,"r")) != NULL)
  else
    error_printf("File not found!\r\n");
} //file_send(void) 1
